...
It's a dataset from Fitbit users who consented to have their personal data tracked. Including physical activity, weight, sleep activity and heart rate. It is arranged in 18 csv files and each one represents a different aspect of the data.

2. Sort and filter the data:  
I first opened the files in  Excel to get a better idea how the data was organized. I decided to separate the date and time in the sleep and hourly steps tables. I did this because I wanted to have the possibility to filter not only by date but also the time that the it was recorded.

3. Importing the dataset:  
I'm using BigQuery for the data exploration, so I uploaded the files with the daily activities, hourly steps, sleep day and weight log. In this analysis I'm focusing on the steps made by the users.
...
* To get not active days I selected the activity days that the total steps and total distance were 0, and sedentary minutes were 1440. That showed me that 15 Ids had at least a not active day.
...
* I summarized the daily activity table in totals per Id. Giving me the sum of total steps, sum of calories, sum of sedentary minutes, sum of active minutes (sum of very, fairly and lightly active minutes) and the sum of the distance.
* After geting the summary I then joined with a table that had counted the total of days that the Ids were active and not active, and the active rate.
* I then used the daily activity, the hourly steps and summary of activity in Tableau for vizualization.

### Analyze and Share

*You can find the Tableau Dasboard [here](https://public.tableau.com/views/bellabeat_case_study/BellabeatDashboard?:language=en-US&:display_count=n&:origin=viz_share_link)*
...
We can see that the day of the week that has the highest average of total steps is Saturday. But when we look at the total of steps by weekday, it shows that Tuesday is the one that has the highest step count.
...
There are some interesting insights looking at this data. But because the sample size is small and we are not sure of the demographics of the participants, the data can be biased. If further analysis is needed, I recommend Bellabeat doing it's own study.
